...
export const createTransaction = async (req, res) => {
    try {
        ...
        if (!plan || !amount || !upiTransactionId) {
            return res.status(400).json({
                message: 'Plan, amount, and UPI Transaction ID are required'
            });
        }

        // Validate UPI Transaction ID — accepts 12-digit numeric ID or UPI ID format
        ...
        if (!isNumericTransactionId && !isUpiId) {
            return res.status(400).json({
                message: 'Invalid format. Enter a 12-digit transaction ID or UPI ID (e.g. username@paytm)'
            });
        }
