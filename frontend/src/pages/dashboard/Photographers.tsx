        ...
        <DashboardLayout userRole="superadmin">
            <div className="p-6 lg:p-8">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-12 border rounded-xl bg-card">No photographers found.</div>
                ) : (
                    <>
                        {/* Mobile Card View */}
                        <div className="md:hidden space-y-4">
                            {photographers.map((user: any) => (
                                <div key={user._id} className="bg-card p-4 rounded-xl border border-border/50 shadow-sm">
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <h3 className="font-semibold text-foreground">{user.name}</h3>
                                            <p className="text-xs text-muted-foreground">{user.email}</p>
                                        </div>
                                        <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium ${user.isBlocked
                                    ...
                                    <div className="grid grid-cols-3 gap-2 mb-4 text-center">
                                        <div className="bg-muted/30 p-2 rounded">
                                            ...
                                            <p className="font-bold">{user.totalEvents || 0}</p>
                                        </div>
                                        <div className="bg-green-50 p-2 rounded">
                                            <p className="text-xs text-green-700 uppercase">Active</p>
                                            <p className="font-bold text-green-700">{user.activeEvents || 0}</p>
                                        </div>
                                        <div className="bg-yellow-50 p-2 rounded">
                                            <p className="text-xs text-yellow-700 uppercase">Inactive</p>
                                            <p className="font-bold text-yellow-700">{user.inactiveEvents || 0}</p>
                                        </div>
                                    </div>
